...
program
  .command("sign_transaction <recipientAddress> <amount>")
  .option("-t --token <tokenAddress>")
  .option("-d --decimals <decimals>")
  .action(async (recipientAddress: string, amount: string, options) => {
    if (recipientAddress == null) {
    ...
    let wallet = getWalletFromConfig();
    await wallet.sign_transaction(recipientAddress, utils.parseUnits(amount, decimals));
  });

program
  .command("init_tx <txHash> <sig_r> <sig_s>")
  .option("-t --token <tokenAddress>")
  .option("-d --decimals <decimals>")
  .action(async (txHash: string, sig_r: string, sig_s: string, options) => {
    let wallet = getWalletFromConfig();
    await wallet.init_tx(txHash, sig_r, sig_s);
  });

program.command("get_tx_status <txHash>").action(async (txHash: string, options) => {
  let body = {
    tx_hash: txHash,
  };
  let res = await get(`get_tx_status`, body);
